...
while True:
    ...
    GPIO.output(7,GPIO.HIGH) if isStop(detections[0,0]) else  GPIO.output(7,GPIO.LOW)
    GPIO.output(12,GPIO.HIGH) if isStop(detections[0,0]) else  GPIO.output(12,GPIO.LOW) 

    image_boxed = draw_bounding_boxes(frame, detections, labels) # Draw bounding boxes
    
    fps = (1/(current_frame_time-previous_frame_time))*1.5
    ...
    cv2.putText(image_boxed, "fps:{}".format(fps), (7, 70), cv2.FONT_HERSHEY_SIMPLEX, 3, (100, 255, 0), 3, cv2.LINE_AA)
